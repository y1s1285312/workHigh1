...
today = date.today()
dropday = today.isoformat().replace('-','.')[2:]
now = time.localtime()
droptime = '{}:{}'.format(now.tm_hour+1,'00')
origin = dropday.replace('.','')+'_ICTIS'
...
for ip in iplists:
    ...
    if score >= 5:
        findlist.append('{}\t{}\t{}\t{}\t{}'.format(dropday,droptime,origin,ip,country))
        totalcount += 1
        print(ip, score)
        continue
