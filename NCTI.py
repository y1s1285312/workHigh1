...
for ip in iplists:
    ...
    if ip.count('.') == 3 :
        ...
        try:
            res = datas['history'][-1]
            countrycode = datas['geo']['country'].upper().replace(' ', '')

            if countrycode not in countrylists.keys() :
                country = countrycode
            else :
    ...
    response = requests.get('https://api.xforce.ibmcloud.com/api/whois/{}'.format(ip), headers=headers)
    ret = response.content.decode()
    datas = json.loads(ret)
    try:
        # res = datas['contact']
        # score = res['score']
        countrycode = datas['contact'][0]['country'].upper().replace(' ', '')
        # print(countrycode)
        country = countrylists[countrycode]
